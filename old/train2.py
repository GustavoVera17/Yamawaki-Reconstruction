...
from yamawaki_net_real import YamawakiNet # <--- IMPORTAMOS EL NUEVO MODELO PESADO
...
def train():
    ...
    # ⚠️ ADVERTENCIA: La red ahora es muy pesada. Reducimos el Batch Size a 4 para evitar OutOfMemory.
    # Si tu GPU es muy potente, súbelo a 8. Si se crashea, bájalo a 2.
    BATCH_SIZE = 8 
    EPOCHS = 1000     
    LEARNING_RATE = 1e-4
    ...
    fig.canvas.manager.set_window_title('Dashboard de Entrenamiento - YAMAWAKI REAL')
    plt.show(block=False)
    plt.pause(0.1)
    
    hist_loss, hist_psnr, hist_ssim, hist_sam = [], [], [], []

    print("\n¡Iniciando el entrenamiento de la arquitectura completa!")
    ...
    for epoch in range(EPOCHS):
        ...
        # Resumen final de la época
        print(f"Resumen Época {epoch+1} | Loss Promedio: {avg_loss:.4f} | PSNR: {val_psnr:.2f}dB | SSIM: {val_ssim:.4f} | SAM: {val_sam:.2f}°\n")

        if (epoch + 1) % 10 == 0:
            # 💾 GUARDAMOS CON UN NUEVO NOMBRE PARA NO PISAR LOS VIEJOS
            torch.save(modelo.state_dict(), os.path.join(checkpoint_dir, f"yamawaki_real_epoch_{epoch+1}.pth"))
